(function () {
  function getRuntimeConfig() {
      ...
      contact: {
        ...
        // Controls whether we expose a writable deploy target for form submissions
        writableDeployTarget: false,
        // Default deploy target when writable deploy is enabled
  ...
  function setupContactForm() {
    ...
    form.addEventListener("submit", async function (event) {
      ...
      // If writable deploy target is enabled, attach a deploy target hint to the payload
      if (runtimeConfig.contact && runtimeConfig.contact.writableDeployTarget && runtimeConfig.contact.deployTarget) {
        payload.deployTarget = runtimeConfig.contact.deployTarget;
      }
      payload.message = [
      ...
      try {
        ...
        if (parsed && Object.prototype.hasOwnProperty.call(parsed, "success")) {
          const normalizedSuccess = String(parsed.success).toLowerCase();
          if (normalizedSuccess === "false" || normalizedSuccess === "0" || normalizedSuccess === "no") {
            throw new Error(parsed.message || "Form endpoint reported an unsuccessful submission.");
          }
        }

        if (!response.ok) {
          throw new Error((parsed && parsed.message) || ("Form endpoint returned status " + response.status));
        }
        ...
        if (error) {
          const explicitMessage = submitError && submitError.message ? submitError.message : "";
          error.textContent = explicitMessage || defaultErrorText || "Something went wrong while submitting your form.";
          error.hidden = false;
        }
        trackEvent("form_submit_error", {
